<div>
    <form action="/bookings" method="POST" class="booking-form">
        ...
        <input type="date" name="date" class="booking-date" min="{{ $minDate }}" max="{{ $maxDate }}" wire:model="date">

        <select name="time" class="booking__time-select" wire:model="time">
            @foreach($timeSlots as $slot)
            <option value="{{ $slot['timeString'] }}" @if($slot['isDisabled']) disabled @endif>{{ $slot['timeString'] }}</option>
            @endforeach
        </select>
        <input type="number" name="number" min="1" max="10" class="booking__number-select" wire:model="number">
